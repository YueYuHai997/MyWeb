...
class Media {
  ...
  containsPoint(screenX, screenY) {
    ...
    const centerY = ((-this.plane.position.y / this.viewport.height) + 0.5) * this.screen.height;
    ...
    const rotation = this.plane.rotation.z;
...
class GalleryApp {
  ...
  getMediaAtPoint(pointer) {
    const local = this.getPointerInContainer(pointer);
    return [...this.medias].reverse().find((media) => media.containsPoint(local.x, local.y)) || null;
  }

  updateCursor(pointer) {
    const media = pointer ? this.getMediaAtPoint(pointer) : null;
    this.container.style.cursor = media ? "pointer" : this.isDown ? "grabbing" : "grab";
  }

  onTouchDown(event) {
    this.isDown = true;
    this.dragDistance = 0;
    this.lastPointer = this.getPointer(event);
    this.scroll.position = this.scroll.current;
    this.start = this.lastPointer.x;
  ...
  onTouchMove(event) {
    ...
    this.dragDistance = Math.max(this.dragDistance, Math.abs(this.start - pointer.x));
    this.scroll.target = this.scroll.position + distance;
    this.container.style.cursor = "grabbing";
  }

  onTouchUp(event) {
    const pointer = this.lastPointer || this.getPointer(event);
    this.isDown = false;
    this.onCheck();

    if (this.dragDistance < 8 && pointer) {
      const media = this.getMediaAtPoint(pointer);
      if (media?.href) {
        window.location.hash = media.href;
      }
    }
